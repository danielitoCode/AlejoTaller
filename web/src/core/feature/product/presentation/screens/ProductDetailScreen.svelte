<script lang="ts">
    ...
    let detailBurst = false;

    function handleFavoriteToggle() {
        const wasLiked = isLiked;
        likedStore.toggle(product.id);
        onFavoriteClick();
        if (!wasLiked) {
            detailBurst = false;
    ...
    {#if showTopBar}
        <header class="header-section">
            ...
            <div class="header-actions">
                <button
                    class="icon-button action-button favorite-detail-btn"
                    class:liked={isLiked}
                    class:burst={detailBurst}
                    type="button"
                    aria-label={isLiked ? "Quitar de me gusta" : "Me gusta"}
                    aria-pressed={isLiked}
                    title={isLiked ? "Me gusta" : "Me gusta"}
                    on:click={handleFavoriteToggle}
                >
                    <span class="heart-icon" aria-hidden="true">
                        <Icon icon={isLiked ? FavoriteFilledRounded : FavoriteBorderRounded} />
                    </span>
                    {#if detailBurst}
...
<style>
    ...
    .favorite-detail-btn.liked {
        color: #ff2d55;
    }

    .favorite-detail-btn.liked :global(svg) {
        color: #ff2d55;
    }

    .favorite-detail-btn .heart-icon {
        display: grid;
        place-items: center;
    }

    .favorite-detail-btn.burst .heart-icon {
    ...
    .favorite-detail-btn .heart-burst {
        position: absolute;
        inset: 0;
        pointer-events: none;
    }

    .favorite-detail-btn .burst-particle {
        position: absolute;
        left: 50%;
        top: 50%;
        width: 6px;
    ...
    .favorite-detail-btn .burst-particle:nth-child(odd) {
        background: #ff7a9a;
    }
    ...
    @keyframes ig-burst {
        ...
        100% {
            opacity: 0;
            transform: rotate(var(--angle)) translateY(-24px) scale(0.2);
        }
    }
    ...
    .carousel-btn-icon { font-size: 1.1rem; line-height: 1; }
    ...
    .carousel-dot:hover { background: var(--md-sys-color-primary); }
    ...
    @media (max-width: 768px) {
        ...
        .carousel-dots { bottom: 10px; gap: 6px; padding: 4px 10px; }
    ...
    .stock-badge-icon :global(svg) { width: 16px; height: 16px; }
    ...
    .price-section.has-promo .price-value { color: var(--md-sys-color-primary); }
    ...
    @media (max-width: 840px) {
        ...
        .product-info-section { padding: 20px 18px 8px; gap: 10px; }
        .description-section { padding: 0 18px 12px; gap: 8px; }
        .bottom-bar { padding: 12px 16px calc(env(safe-area-inset-bottom, 0px) + 12px); }
        .icon-button { width: 44px; height: 44px; }
        .cart-action :global(.m3-container) { min-height: 52px; }
    }
    ...
    @media (max-width: 480px) {
        ...
        .product-image-section { border-radius: 0 0 24px 24px; }
        .detail-copy-card { border-radius: 24px 24px 0 0; }
        .product-name { font-size: 1.28rem; }
        .price-value { font-size: 1.5rem; }
        .product-description { font-size: 0.92rem; line-height: 1.55; }
        .icon-button { width: 42px; height: 42px; }
        .share-label { display: none; }
        .product-info-section { padding: 16px 14px 6px; }
        .description-section { padding: 0 14px 10px; }
    }
    ...
    @media (min-width: 841px) {
        ...
        .product-image-section { border-radius: 0; }
        .detail-copy-card { margin-top: 0; border-radius: 0; }
        .bottom-bar { border-radius: 0 0 28px 28px; }
    }
