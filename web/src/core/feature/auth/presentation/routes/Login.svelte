<script lang="ts">
    ...
    async function handleGoogleProfile(profile: GoogleIdTokenProfile) {
        ...
        try {
            ...
            try {
                ...
                const currentPhoto = typeof current?.photo_url === "string" ? current.photo_url.trim() : "";
                ...
                const authContext = { userId, email: sanitizedProfile.email, provider: "google" as const };
            ...
            authFlowStore.setError(error, { email: sanitizedProfile.email, provider: "google" });
            toastStore.error(error);
        } finally {
            loading = false;
        }
    ...
    async function linkGoogleAccount() {
        ...
        if (!linkPassword.trim()) { linkError = "Ingresa tu contrasena actual."; return; }
        loading = true;
        linkError = null;
        error = null;
        try {
            ...
            linkOpen = false;
            linkPassword = "";
            const authContext = { userId, email: googleProfile.email, provider: "google" as const };
            if (await maybeHandleAdminChoice(currentUser, authContext)) return;
            completeClientLogin(authContext);
        } catch (e: any) {
            const code = typeof e?.code === "number" ? e.code : null;
            linkError = code === 401 ? "Contrasena incorrecta." : (e instanceof Error ? e.message : "No se pudo vincular la cuenta.");
            authFlowStore.setError(linkError, { email: googleProfile.email, provider: "google" });
    ...
    async function registerStoreFromGoogle(profile: GoogleIdTokenProfile) {
        ...
        try {
            ...
            const current = await authContainer.useCases.accounts.getCurrentUser();
            const authContext = { userId: current.id ?? "", email: sanitizedProfile.email ?? "", provider: "google" as const };
            if (await maybeHandleAdminChoice(current, authContext)) return;
            completeClientLogin(authContext);
        } catch (e: any) {
            const code = typeof e?.code === "number" ? e.code : null;
    ...
    function closeGoogleFrame() { googleFrameOpen = false; googleAuthSrc = ""; }
    function closeRegisterFrame() { registerFrameOpen = false; googleRegisterSrc = ""; }

    async function handleGoogleFrameMessage(data: any) {
        if (!data || typeof data !== "object") return;
        if (data.type === "google-cancel") { closeGoogleFrame(); return; }
    ...
    async function handleRegisterFrameMessage(data: any) {
        if (!data || typeof data !== "object") return;
        if (data.type === "google-register-cancel") { closeRegisterFrame(); return; }
            ...
            <Card variant="filled">
                <div class="link-card-content">
                    <div class="link-copy">
                        ...
                        <p>Esta cuenta ya existe con una contrasena personalizada. Usa tu password actual para vincular Google y continuar.</p>
                    </div>
                    <div class="field-wrap">
                        <TextFieldOutlined label="Contrasena actual" bind:value={linkPassword} type="password" leadingIcon={LockOutline} enter={linkGoogleAccount} />
                    </div>
                    {#if linkError}<p class="error-copy">{linkError}</p>{/if}
...
<style>
    ...
    .login-screen-mobile {
        ...
        background:
            radial-gradient(circle at 50% 12%, color-mix(in srgb, var(--md-sys-color-primary) 18%, transparent), transparent 32%),
            linear-gradient(
                180deg,
    ...
    .login-brand { display: grid; justify-items: center; text-align: center; gap: 8px; align-content: center; min-width: 0; }
    .login-brand h2 { margin: 0; font-size: clamp(1.5rem, 6vw, 2.5rem); line-height: 1.1; }
    .login-brand p { margin: 0; color: var(--md-sys-color-on-surface-variant); font-size: clamp(0.95rem, 3.8vw, 1.08rem); }
    ...
    .login-card { width: 100%; max-width: 420px; margin-inline: auto; min-width: 0; }
    .login-card-content { padding: 16px; display: grid; gap: 12px; min-width: 0; }
    ...
    .field-wrap :global(input) { min-height: 56px; color: var(--md-sys-color-on-surface); }
    .field-wrap :global(label) { color: var(--md-sys-color-on-surface-variant); }
    .field-wrap :global(.layer) { border-color: var(--md-sys-color-outline); border-width: 1.5px; }
    .field-wrap :global(.leading), .field-wrap :global(.trailing) { color: var(--md-sys-color-on-surface-variant); }
    .field-wrap :global(input:focus ~ .layer) { border-color: var(--md-sys-color-primary); }
    .field-wrap :global(input:focus ~ label) { color: var(--md-sys-color-primary); }
    .action-row :global(.m3-container) { width: 100%; min-width: 0; max-width: 100%; }
    .action-row :global(.m3-container.m) { padding-inline: clamp(1.35rem, 4vw, 1.85rem); }
    .btn-content {
        width: 100%; min-width: 0; display: inline-flex; align-items: center; justify-content: center;
        gap: 8px; font-weight: 500; padding: 12px 0;
    }
    .google-icon { width: 18px; height: 18px; object-fit: contain; flex: 0 0 auto; }
    .auth0-hint { margin: 0; text-align: center; font-size: 0.8rem; color: var(--md-sys-color-on-surface-variant); }
    .error-copy { margin: 4px 0 0; color: var(--md-sys-color-error); font-size: 0.9rem; text-align: center; }
    .signature { display: grid; justify-items: center; gap: 2px; color: var(--md-sys-color-on-surface-variant); font-size: 0.78rem; padding-bottom: 2px; }
    .signature p { margin: 0; }
    .signature-desktop { display: none; }
    .link-modal { position: fixed; inset: 0; z-index: 1200; display: grid; place-items: center; padding: 16px; }
    .link-scrim { position: absolute; inset: 0; border: 0; background: color-mix(in srgb, black 44%, transparent); backdrop-filter: blur(6px); }
    .link-panel { position: relative; z-index: 1; width: min(460px, calc(100vw - 24px)); }
    .link-panel :global(.m3-container.filled) {
        border-radius: 24px;
        background: var(--md-sys-color-surface-container-high);
        --m3v-background: var(--md-sys-color-surface-container-high);
    }
    .link-card-content { display: grid; gap: 14px; padding: 18px; }
    .link-copy { display: grid; gap: 6px; }
    .link-copy h3, .link-copy p { margin: 0; }
    .link-copy p { color: var(--md-sys-color-on-surface-variant); line-height: 1.45; }
    .link-actions { display: flex; flex-wrap: wrap; justify-content: flex-end; gap: 10px; }
    @media (max-height: 760px) {
        .login-screen-mobile { gap: 8px; padding-top: max(10px, calc(env(safe-area-inset-top) + 4px)); padding-bottom: max(10px, calc(env(safe-area-inset-bottom) + 6px)); overflow-y: auto; }
        .login-brand h2 { font-size: clamp(1.7rem, 7vw, 2.2rem); }
        .login-card-content { gap: 10px; padding: 12px; }
        .field-wrap :global(.m3-container) { height: 54px; }
        .field-wrap :global(input) { min-height: 54px; }
    }
    @media (min-width: 800px) {
        .login-screen-mobile {
            max-width: 1180px; margin: 0 auto;
            padding: max(18px, calc(env(safe-area-inset-top) + 10px)) 28px max(18px, calc(env(safe-area-inset-bottom) + 10px));
            display: grid; grid-template-columns: minmax(320px, 0.95fr) minmax(380px, 0.9fr);
            align-items: center; justify-content: center; justify-items: center; gap: clamp(24px, 4vw, 56px);
        }
        .login-card { max-width: 548px; width: 100%; }
        .login-brand { width: 100%; max-width: 420px; justify-items: center; text-align: center; align-content: center; min-height: 0; gap: 12px; }
        .login-brand h2 { font-size: clamp(2.6rem, 4.5vw, 4rem); }
        .login-brand p { font-size: clamp(1.05rem, 1.6vw, 1.3rem); }
        .login-indicator-wrap { width: 148px; height: 148px; }
        .login-logo { width: 68px; height: 68px; }
        .signature { display: none; }
        .signature-desktop { display: grid; justify-items: center; font-size: 0.68rem; gap: 1px; opacity: 0.82; }
        .signature-desktop p { margin: 0; }
    }
    @media (max-width: 360px) {
        .login-indicator-wrap { width: 100px; height: 100px; }
    }
</style>
