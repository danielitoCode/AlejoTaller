<script lang="ts">
    ...
    const useAuth0 = isExternalAuthProvider(); // Clerk o Auth0
                ...
                if (auth) {
                    try {
                        ...
                        if (session) {
                            ...
                            if (import.meta.env.DEV) logNavAuthCheck(true, false, "idp-nested-session");
                        ...
                        if (import.meta.env.DEV) logNavAuthCheck(false, true, "idp-nested-error");
                    }
                }
                // No forzar invitado si Splash ya autenticó (carrera Clerk post-redirect)
                if (!get(sessionStore).isGuest) {
                    if (import.meta.env.DEV) logNavAuthCheck(true, false, "idp-nested-keep-auth");
                    return resolvedUser ?? { name: "Usuario" };
                }
                forceVisitorMode();
                resolvedUser = { name: "Visitante" };
                if (import.meta.env.DEV) logNavAuthCheck(false, true, "idp-nested-guest");
