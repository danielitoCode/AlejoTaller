<script lang="ts">
    ...
    $: if (hashSyncReady && !suppressHashSync && typeof window !== "undefined") {
        ...
        const nextHash = buildHomeHash(
            currentPath as typeof dashboard.path | typeof buy.path | typeof buyConfirm.path | typeof reservation.path | typeof reservationDetail.path | typeof profile.path | typeof settingsRoute.path | typeof productDetail.path | typeof support.path | typeof supportDetail.path,
            ...
            <div class="fab-stack">
                {#if fabOpen}
                    <div class="fab-menu" aria-label="Navegacion rapida">
                        ...
                        {#if isGuestSession}
                            <div class="fab-item-row">
                                <Button class="fab-label" variant="filled" size="m" onclick={handleRequestLogin}>
                                    Iniciar sesión
                                </Button>
                                <button
                                    class="fab-mini"
                                    type="button"
                                    on:click={handleRequestLogin}
...
<style>
    ...
    .panel-promo :global(.promo-nav-btn) {
        width: 100%;
        max-width: 100%;
        box-sizing: border-box;
        box-shadow:
    ...
    .rail-wrap :global(.m3-container:has(> .rail.open:not(.modal))) { width: 100%; }
    ...
    .rail-wrap :global(.label) { overflow: visible; }
    ...
    .mobile-title strong { font-size: 1.04rem; }
    .mobile-title span {
        color: var(--md-sys-color-on-surface-variant);
        font-size: 0.8rem;
        overflow: hidden;
    ...
    .fab-layer { display: none; }
    .promo-float-host { display: none; }
    ...
    @media (max-width: 840px) {
        ...
        .top-mobile :global(button) {
            flex-shrink: 0;
            border-radius: 999px;
            font-weight: 700;
        }
        .compact-only { display: block; }
        .promo-float-host { display: block; }
        .fab-layer {
            display: block;
            position: fixed;
            inset: 0;
