...
fun InternalNavigationWrapper(
    ...
    val likedProductsStore: LikedProductsStore = koinInject()
    val likedProductIds by likedProductsStore.likedIds.collectAsStateWithLifecycle()
                ...
                entry<InternalRoutesKey.Home>(
                    ...
                    ProductScreen(
                        ...
                        onFavoriteClick = { productId ->
                            likedProductsStore.toggle(productId)
                        },
                ...
                entry<InternalRoutesKey.ProductDetail>(
                    ...
                    if (resolvedProduct != null) {
                        ProductDetailScreen(
                            ...
                            isLiked = resolvedProduct!!.id in likedProductIds,
                            onBackClick = { backStack.navigateBack() },
                            onFavoriteClick = {
                                likedProductsStore.toggle(resolvedProduct!!.id)
                            },
                            onAddToCartClick = {
                                if (isGuest) {
                                    toasterViewModel.showMessage(
                                    ...
                                    saleViewModel.initiatePayment(
                                        ...
                                        onReadyToPay = { saleId, checkoutUrl ->
                                            ...
                                            Log.i(
                                                ...
                                                "event=payment_redirect saleId=$saleId channel=${paymentChannel.name} url=$checkoutUrl host=${checkoutUri.host} scheme=${checkoutUri.scheme}"
                                            )
                                        ...
                                        onFail = { error ->
                                            ...
                                            toasterViewModel.dismissMessage("sale_charge")
                                            toasterViewModel.showMessage(
                                                "No se pudo procesar el pago (${paymentChannel.name}): $error",
                                                ToastType.Error
                                            )
                                        }
                                    )
