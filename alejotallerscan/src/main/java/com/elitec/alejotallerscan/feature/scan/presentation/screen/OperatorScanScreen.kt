...
fun OperatorScanScreen(
    ...
    OperatorScreen(
        ...
        subtitle = "Escanea el QR dentro del marco o carga manualmente el ID guardado en Appwrite."
    ) {
        Column(verticalArrangement = Arrangement.spacedBy(16.dp)) {
            Card(
                ...
                Column(
                    ...
                    Text("Escaneo QR", style = MaterialTheme.typography.titleMedium)
            ...
            Card(
                ...
                Column(
                    ...
                    Text("Carga manual", style = MaterialTheme.typography.titleMedium)
                    OutlinedTextField(
                        value = manualCode,
                ...
                Card(
                    ...
                    colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.surface)
                        ...
                        Column(
                            ...
                            Text("Venta cargada", style = MaterialTheme.typography.titleMedium)
                            uiState.parsedPayload?.let { parsed ->
                                Text("Datos detectados en QR", style = MaterialTheme.typography.titleSmall)
                                Text("ID QR: ${parsed.saleId}")
                                Text("Usuario QR: ${parsed.userId ?: "No disponible"}")
                                Text("Importe QR: ${parsed.amount?.let { "$$it" } ?: "No disponible"}")
                                if (parsed.items.isNotEmpty()) {
                                    parsed.items.forEach { item ->
                                        Text(
                                            qrItemLabel(item, sale),
                                            color = MaterialTheme.colorScheme.onSurfaceVariant
                                        )
                                    }
                                }
                            ...
                            Text("Datos validados en Appwrite", style = MaterialTheme.typography.titleSmall)
                            Text("ID pedido: ${sale.id}")
                            Text("ID usuario: ${sale.userId}")
                            Text("Nombre cliente: ${sale.customerName ?: "No disponible"}")
